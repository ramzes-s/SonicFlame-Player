...
class ArtistCardWidget(QWidget):
    ...
    # ---- textColor Property for Animation ----
    @Property(QColor)
    ...
    # ---- countOpacity Property for Animation ----
    @Property(float)
    ...
    # ---- borderColor Property for Animation ----
    @Property(QColor)
    ...
    def update_accent_color(self):
        """Updates the accent color used for animations by re-reading the global config."""
    ...
    def enterEvent(self, event):
        """Handle mouse enter event to start animations."""
    ...
    def leaveEvent(self, event):
        """Handle mouse leave event to reverse animations."""
    ...
    def paintEvent(self, event):
        """Custom paint event to draw the collage and text."""
        painter = QPainter(self)
        painter.setRenderHint(QPainter.Antialiasing)

        # Draw the collage background (scaled to fit)
        if not self._collage.isNull():
            painter.drawPixmap(self.rect(), self._collage)

        # --- Draw Gradient Scrim for text readability ---
        ...
        painter.drawRect(self.rect().adjusted(0, 0, -1, -1)) # Adjust for 1px line
