...
from musicplayer.ui.artist_processing_worker import ArtistProcessingWorker
from musicplayer.ui.artist_card_widget import ArtistCardWidget
...
class ArtistViewWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._col_count = 0
        self.resizeEvent(None) # Initial call

    def resizeEvent(self, event):
        """Recalculate column count on resize."""
        # Card has min-width of 180, plus spacing
    ...
    def load_if_needed(self):
        """
        Loads artist data if it hasn't been loaded yet.
        Checks cache status and decides whether to load from cache or
        start the background worker to rebuild it.
        """
        if self._is_loaded:
            return
        ...
        if db.get_artists_cache_status():
            ...
            # Cache is stale or doesn't exist, rebuild
            self._rebuild_cache()
    
    def _load_from_cache(self):
        """Loads all artists from the cache and populates the grid."""
    ...
    def _rebuild_cache(self):
        """Starts the worker to rebuild the artist cache."""
    ...
    def _add_artist_card_to_grid(self, artist_data: dict):
        """Creates an ArtistCardWidget and adds it to the grid."""
    ...
    def _on_worker_finished(self):
        """Called when the worker is done processing."""
        self._is_loaded = True
        # Worker is a child of self, should be garbage collected
        self.worker = None

    def update_accent_color(self):
        """Propagate accent color change to all child card widgets."""
        for i in range(self._grid_layout.count()):
            item = self._grid_layout.itemAt(i)
            if item and item.widget():
                # Check if the widget is an ArtistCardWidget
