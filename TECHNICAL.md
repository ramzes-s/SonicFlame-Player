...
#### `ui/library_dialog.py` — LibraryDialog
...
- **Вкладка "Треки"**:
    ...
    - **Звезда настроения**: Новая колонка "★" с кастомным делегатом `MoodStarDelegate`.
...
- **Вкладка "Исполнители"**:
    - Содержит `ArtistViewWidget` для отображения сетки исполнителей.
    - Загрузка данных для этой вкладки происходит только при ее первом открытии.
...
#### `ui/artist_view_widget.py` — Представление "Артисты"
Основной виджет для вкладки "Исполнители".
- **Состояния**: Использует `QStackedWidget` для отображения либо спиннера "Загрузка...", либо сетки с карточками исполнителей.
- **Кеширование**: При первой загрузке вызывает `load_if_needed()`, которая проверяет статус кеша в БД. Если кеш актуален (`db.get_artists_cache_status()`), загружает данные из него (`db.get_cached_artists()`). Если нет — запускает `ArtistProcessingWorker` для пересборки.
- **Адаптивная сетка**: Отображает карточки в `QGridLayout`, которая автоматически меняет количество колонок в зависимости от ширины окна.
- **Сигналы**: При клике на карточку исполнителя пересылает сигнал `artist_play_requested` в `MainWindow` через `LibraryDialog` и IPC.

#### `ui/artist_card_widget.py` — Карточка исполнителя
Виджет для отображения одного исполнителя в сетке.
- **Визуализация**: Показывает сгенерированный коллаж из 4 обложек, имя артиста и количество треков.
- **Анимация**: При наведении курсора плавно анимирует цвет текста и рамки, используя `QPropertyAnimation`.

#### `ui/artist_processing_worker.py` — Обработчик данных артистов
Фоновый `QThread` для подготовки данных.
- **Агрегация**: Читает всю библиотеку, группирует треки по основному исполнителю (первому в списке, если их несколько).
- **Фильтрация**: Игнорирует исполнителей, у которых в библиотеке менее 3 треков.
- **Генерация коллажей**: Для каждого исполнителя берет до 4-х обложек и создает из них единый коллаж 2x2. Коллажи сохраняются в `.cache/artist_collages/`.
- **Динамическое добавление**: По мере готовности данных для каждого исполнителя отправляет сигнал `artist_ready`, что позволяет UI не ждать полной обработки, а показывать карточки по одной.
- **Сохранение в кеш**: После завершения обработки всех исполнителей, сохраняет результат в таблицу `artists_cache` в БД.
