...
├── __init__.py          # Точка входа: register(hub) (обязательно), опционально unregister(hub)
...
#### `add_context_submenu(label, items)`

Добавляет вложенное подменю в контекстное меню плейлиста.

| Параметр | Тип | Описание |
|----------|-----|----------|
| `label` | str | Название подменю. |
| `items` | list | Список кортежей `(item_text, callback)`, где `callback` получает `view_index` кликнутого трека. Используйте `None` вместо кортежа для разделителя. |

**Пример:**
```python
hub.add_context_submenu("Действия", [
    ("Пункт 1", lambda vi: print(f"clicked track index {vi}")),
    None,  # разделитель
    ("Пункт 2", lambda vi: print(f"track {vi}")),
])
```
...
inner.addWidget(config, 1)  # stretch factor 1 — виджет заполняет всё пространство
...
>
> **Данные** плагина (плейлисты, кэши, пользовательские файлы) следует хранить в своей папке `plugins/<name>/`. Используйте `hub.get_data_dir()` для получения пути — папка создаётся автоматически при первом вызове.
...
Возвращает путь к директории данных плагина — `plugins/<name>/`. Папка создаётся автоматически при первом вызове. Используйте этот метод для хранения **данных** плагина (плейлисты, кэши, пользовательские файлы), в отличие от **настроек** (маленькие key-value конфиги), которые хранятся через `get_plugin_settings()` / `save_plugin_settings()` в `.cache/plugins.json`.

**Пример:**

```python
data_dir = hub.get_data_dir()
playlists_file = data_dir / "my_playlists.json"
```
...
- При отключении плагина `PluginManager.unregister_plugin()` очищает все зарегистрированные UI-элементы: кнопки на боковой панели, пункты контекстного меню, подменю и колбэк `context_refresh`.
- Плагин может определить функцию `unregister(hub)` для собственной очистки (закрытие потоков, сохранение состояния). Она вызывается автоматически при отключении:
  ```python
  def unregister(hub):
      """Опциональная очистка при выключении плагина."""
...
| `add_context_submenu(label, items)` | Подменю в контекстном меню |
...
| `get_data_dir()` | Путь к папке данных плагина `plugins/<name>/` |
...
11. Плагин **обязан** использовать **относительные импорты** для доступа к своим подмодулям (`from .pages.xxx import Y`), а не абсолютные через `plugins.<name>.<xxx>`. При загрузке плагин импортируется как `<entry>` (например, `library_stats`), а не как `plugins.<entry>`. Абсолютный импорт вида `from plugins.library_stats.pages.xxx` создаёт второй экземпляр модуля в `sys.modules` с другим именем, что в frozen exe приводит к `ModuleNotFoundError` — модуль регистрируется, но `set_settings_widget()` не вызывается, и кнопка настройки не появляется.

12. **Настоятельно рекомендуется** добавлять в корень плагина файл `README.md` с кратким описанием назначения, структуры и особенностей плагина. Файл не является обязательным для загрузки, но помогает другим разработчикам (и вам в будущем) быстро понять устройство плагина без чтения исходного кода. Примерное содержание: назначение, структура папок, ключевые классы/функции, известные ограничения.
